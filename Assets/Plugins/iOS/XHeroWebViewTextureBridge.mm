...
static NSString *XHeroVideoPatchScript(void) {
    ...
    [script appendString:
         ...
         "function xheroPrepareVideo(){"
           ...
           // Muted autoplay is substantially more reliable on iOS. Unity can request
           // volume/unmute after the first valid frame through XHeroWV_Evaluate.
         ...
         "if(window.xheroVideoCanvasLoop!==true){"
           ...
           "(function xheroLoop(){"
             "try{"
               ...
               "if(cv&&video&&video.readyState>=2){"
                 ...
                 "if(quality&&quality.totalVideoFrames>0){"
                   ...
                   "}else if(now-window.xheroFrameTime>=1000){"
                     "var measured=(quality.totalVideoFrames-window.xheroFrameCount)*1000/(now-window.xheroFrameTime);"
                     "if(measured>=1&&measured<=120){window.xheroEstimatedFps=measured;}"
                     "window.xheroFrameTime=now;"
                     "window.xheroFrameCount=quality.totalVideoFrames;"
                   "}"
                 "}"
               ...
               "}"
             "}catch(e){"
               "window.xheroCanvasError=String(e&&e.message?e.message:e);"
               "window.xheroCanvasErrorCount=(window.xheroCanvasErrorCount||0)+1;"
             "}"
         ...
         "return 'ok';"
       "}catch(e){"
         "return 'patch-error:'+String(e&&e.message?e.message:e);"
       "}"
     "})()"
    ];
