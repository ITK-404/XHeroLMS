...
namespace MegaBook
{
	...
	public class MegaBookBuilder : MonoBehaviour
	{
		...
		GameObject MakePageObject(MegaBookPage page, int pnum)	//, Material mat)
		{
			...
			//Debug.Log("pindex " + pageindex + " params " + pageparams.Count);
		...
		Mesh CreatePageMesh(MegaBookPage page, float width, float length, float height, int widthsegs, int lengthsegs, int heightsegs)
		{
			...
			MegaBookPageParams pp = pageparams[page.pnum % pageparams.Count];
		...
		void Update()
		{
			...
			if ( Flip != page )
			{
				if ( Application.isPlaying )
				{
                    ...
                    //Flip = Mathf.SmoothDamp(Flip, page, ref turnspd, turntime, float.PositiveInfinity, Time.unscaledDeltaTime);
				}
				else
					Flip = page;
