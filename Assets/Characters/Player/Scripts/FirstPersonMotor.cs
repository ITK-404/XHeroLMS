...
public class FirstPersonMotor : MonoBehaviour
{
    ...
    [Header("Speed")]
    public float walk = 4.2f, sprint = 6.8f;
    ...
    void Update()
    {
        ...
        if (wishDir.sqrMagnitude > 0) planar += Vector3.ClampMagnitude(delta, a * Time.deltaTime);
        else
        {
        ...
        // Gravity
        if (grounded && velocity.y < 0) velocity.y = -slopeStick;
        else
        {
