...
namespace GPUInstancerPro.TerrainModule
{
    ...
    public class GPUIDetailManager : GPUITerrainManager<GPUIDetailPrototypeData>
    {
        ...
        [NonSerialized]
        private Action<GPUIDataBuffer<uint>> _processCounterDataCallback;
        [NonSerialized]
        ...
        private void UpdateDetailMatrices(GPUICameraData cameraData)
        {
            ...
            int cameraKey = cameraData.ActiveCamera.GetInstanceIDGPUI();
        ...
        private void ProcessCounterData(GPUIDataBuffer<uint> buffer)
        {
        ...
        public int AddDetailPrototype(DetailPrototype detailPrototype)
        {
            ...
            if (_prototypes != null && _prototypeDataArray != null)
            {
        ...
        {
            return GPUIUtility.GenerateHash(this.GetInstanceIDGPUI(), prototypeIndex);
        }
        ...
        public void Editor_UpdateBakedDetailTextures(bool showDialog = false)
        {
            if (GetActiveTerrainCount() == 0)
                return;
            if (showDialog && !EditorUtility.DisplayDialog(
                "Update Baked Detail Textures",
                "Do you wish to update all baked detail textures?\nThis action will overwrite all referenced detail textures on the terrains.",
                "Update", "Cancel"))
                return;
            foreach (var gpuiTerrain in GetActiveTerrainValues())
            {
                if (gpuiTerrain != null && gpuiTerrain.IsBakedDetailTextures() && gpuiTerrain is GPUITerrainBuiltin terrainBuiltin)
                {
                    terrainBuiltin.Editor_EnableBakedDetailTextures();
                    terrainBuiltin.Editor_SaveDetailRenderTexturesToBakedTextures();
                    EditorUtility.SetDirty(terrainBuiltin);
                }
            }
        }

        [ContextMenu("Bake Detail Textures For All")]
        public void Editor_BakeDetailTexturesForAllTerrains()
        {
            if (GetActiveTerrainCount() == 0)
                return;
            foreach (var gpuiTerrain in GetActiveTerrainValues())
            {
                if (gpuiTerrain != null && gpuiTerrain is GPUITerrainBuiltin terrainBuiltin)
                {
                    terrainBuiltin.Editor_EnableBakedDetailTextures();
                    terrainBuiltin.Editor_SaveDetailRenderTexturesToBakedTextures();
                    EditorUtility.SetDirty(terrainBuiltin);
        ...
        [ContextMenu("Delete All Baked Detail Textures")]
        public void Editor_DeleteDetailTexturesForAllTerrains()
        {
            if (GetActiveTerrainCount() == 0)
                return;
            if (!EditorUtility.DisplayDialog(
                "Delete Baked Detail Textures",
                "Do you wish to delete all baked detail textures?\nThis action will delete all referenced detail textures on the terrains and cannot be undone.",
                "Delete",
                "Cancel"))
                return;
            foreach (var gpuiTerrain in GetActiveTerrainValues())
            {
                if (gpuiTerrain != null && gpuiTerrain is GPUITerrainBuiltin terrainBuiltin)
                {
                    terrainBuiltin.Editor_DeleteBakedDetailTextures(false);
                    EditorUtility.SetDirty(terrainBuiltin);
                }
            }
        }
        ...
        internal class GPUIDetailUpdateData : GPUIDataBuffer<uint>
        {
            ...
            public void ProcessGPUReadback(GPUIDetailManager detailManager)
            {
                if (detailManager._runtimeRenderKeys == null)
                    return;
                NativeArray<uint> requestedData = GetRequestedData();
                ...
                for (int i = 0; i < prototypeCount && i < requestedDataLength; i++)
                {
                    ...
                    {
                        int instanceCount = (int)requestedData[i];
