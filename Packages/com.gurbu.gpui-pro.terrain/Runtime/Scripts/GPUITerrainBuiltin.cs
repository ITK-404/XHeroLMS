...
namespace GPUInstancerPro.TerrainModule
{
    ...
    public class GPUITerrainBuiltin : GPUITerrain
    {
        ...
        public void Editor_DeleteBakedDetailTextures(bool showDialog = true)
        {
            if (showDialog && !EditorUtility.DisplayDialog("Delete Baked Density Textures", "Do you wish to delete the generated detail density textures?", "Yes", "No"))
                return;
            ...
            if (_bakedDetailTextures != null)
            {
                for (int i = 0; i < _bakedDetailTextures.Length; i++)
                {
                    if (_bakedDetailTextures[i] != null &&
                        AssetDatabase.Contains(_bakedDetailTextures[i]) &&
                        _bakedDetailTextures[i].name.Contains(GPUITerrainConstants.NAME_SUFFIX_DETAILTEXTURE))
                    {
                        string assetPath = AssetDatabase.GetAssetPath(_bakedDetailTextures[i]);
                        ...
                        if (!string.IsNullOrEmpty(folderPath) &&
                            ...
                            Path.GetFileName(folderPath) == GPUITerrainConstants.FOLDER_BAKED_DETAIL_TEXTURES)
                        {
                            string[] remainingAssets = AssetDatabase.FindAssets("", new[] { folderPath });

                            if (remainingAssets == null || remainingAssets.Length == 0)
                            {
        ...
        {
            if (Application.isPlaying || !_isBakedDetailTextures)
                return;

            string folderPath = _terrain.terrainData.GetAssetFolderPath();
            if (string.IsNullOrEmpty(folderPath))
                folderPath = "Assets/";
            folderPath += GPUITerrainConstants.FOLDER_BAKED_DETAIL_TEXTURES + "/";
            string fileName = null;

            int detailCount = _terrain.terrainData.detailPrototypes.Length;
            for (int detailLayer = 0; detailLayer < detailCount; detailLayer++)
            {
                if (_bakedDetailTextures[detailLayer] != null)
                {
                    string existingTexturePath = AssetDatabase.GetAssetPath(_bakedDetailTextures[detailLayer]);
                    if (!string.IsNullOrEmpty(existingTexturePath))
                    {
                        folderPath = GPUIUtility.GetFolderPath(existingTexturePath);
                        fileName = Path.GetFileNameWithoutExtension(existingTexturePath);
                    }
                }
                _bakedDetailTextures[detailLayer] = GPUITerrainUtility.SaveDetailDensityTexture(GetDetailDensityTexture(detailLayer), folderPath, fileName);
            }
        }
