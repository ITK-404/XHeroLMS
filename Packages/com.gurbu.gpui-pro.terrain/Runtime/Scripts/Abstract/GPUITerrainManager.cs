...
namespace GPUInstancerPro.TerrainModule
{
    public abstract class GPUITerrainManager<T> : GPUIManagerWithPrototypeData<T> where T : GPUIPrototypeData, new()
    {
        ...
        private void UpdateActiveTerrains()
        {
            ...
            {
                if (gpuiTerrain != null)
                    _gpuiTerrainsDict[gpuiTerrain.GetInstanceIDGPUI()] = gpuiTerrain;
            }
#if UNITY_EDITOR
        ...
        {
            if (IsInitialized)
                _toRemoveActiveTerrains.Add(gpuiTerrain.GetInstanceIDGPUI()); // we need to reset the data before waiting for update, the terrain can be changed and readded until update is executed
