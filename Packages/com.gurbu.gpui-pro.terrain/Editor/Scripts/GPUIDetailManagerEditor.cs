    ...
    public class GPUIDetailManagerEditor : GPUITerrainManagerEditor<GPUIDetailPrototypeData>
    {
        ...
        protected override bool TryGetPreviewForPrototype(int prototypeIndex, GPUIPreviewDrawer previewDrawer, out Texture2D preview)
        {
            ...
            if (detailPrototypeData != null && detailPrototypeData.detailTexture != null)
            {
                ...
                if (!GPUIPreviewCache.TryGetPreview(detailPrototypeData.detailTexture.GetInstanceIDGPUI(), out preview) && previewDrawer != null)
                {
                    if (previewDrawer.TryGetPreviewForPrototype(null, PREVIEW_ICON_SIZE, detailPrototypeData.detailTexture, out preview))
                    {
                        GPUIPreviewCache.AddPreview(detailPrototypeData.detailTexture.GetInstanceIDGPUI(), preview);
        ...
        protected override void DrawPrototypeButtons()
        {
            ...
            if (!Application.isPlaying && _gpuiDetailManager.GetTerrainCount() > 0)
            {
                bool hasBakedDetailTextures = false;
                for (int i = 0; i < _gpuiDetailManager.GetTerrainCount(); i++)
                {
                    var gpuiTerrain = _gpuiDetailManager.GetTerrain(i);
                    if (gpuiTerrain != null && gpuiTerrain.IsBakedDetailTextures() && gpuiTerrain is GPUITerrainBuiltin)
                    {
                        hasBakedDetailTextures = true;
                        break;
                    }
                }

                if (hasBakedDetailTextures)
                {
                    GPUIEditorTextUtility.TryGetGPUIText("updateBakedDetailTextures", out GPUIEditorTextUtility.GPUIText gpuiText);
                    Button updateBakedTexturesButton = new()
                    {
                        text = gpuiText.title,
                        focusable = false,
                        tooltip = gpuiText.tooltip,
                    };
                    updateBakedTexturesButton.AddToClassList("gpui-pre-prototype-button");
                    updateBakedTexturesButton.RegisterCallback<ClickEvent>((evt) => { _gpuiDetailManager.Editor_UpdateBakedDetailTextures(true); DrawManagerSettings(); DrawPrototypeButtons(); });
                    _prePrototypeButtonsVE.Add(updateBakedTexturesButton);
        ...
        {
            GPUIDetailManager detailManager = GPUIUtility.FindFirstObjectByType<GPUIDetailManager>();
            GameObject go;
            if (detailManager == null)
            {
                go = new GameObject("GPUI Detail Manager");
                detailManager = go.AddComponent<GPUIDetailManager>();
                detailManager.AddTerrains(GPUIUtility.FindObjectsByType<GPUITerrain>());
