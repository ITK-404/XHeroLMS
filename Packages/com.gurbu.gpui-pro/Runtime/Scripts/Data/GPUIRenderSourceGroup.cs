...
//#endif
        ...
        public static int GetKey(int prototypeKey, GPUIProfile profile, int groupID, List<string> shaderKeywords)
        {
            if (shaderKeywords == null || shaderKeywords.Count == 0)
                return GPUIUtility.GenerateHash(prototypeKey, profile.GetInstanceIDGPUI(), groupID);
            shaderKeywords.Sort();
            return GPUIUtility.GenerateHash(prototypeKey, profile.GetInstanceIDGPUI(), groupID, string.Concat(shaderKeywords).GetHashCode());
        }

        public int GetRenderSourceKey(UnityEngine.Object source)
        {
            return GPUIUtility.GenerateHash(source.GetInstanceIDGPUI(), Key);
        }
    ...
    public class GPUIRenderSource : IGPUIDisposable
    {
        ...
        {
            return GPUIUtility.GenerateHash(source.GetInstanceIDGPUI(), renderSourceGroup.Key);
        }
    }
