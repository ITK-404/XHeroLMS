            ...
            foreach (GPUIRenderSourceGroup rsg in GPUIRenderingSystem.Instance.RenderSourceGroupProvider.Values)
            {
                ...
                else
                {
                    ...
                    for (int s = 0; s < rsCount; s++)
                    {
                        ...
                        int count = rs.instanceCount;
                        int start = rs.bufferStartIndex;

                        if (count <= 0 || start < 0)
                            continue;

                        bool isFull = count == rs.bufferSize;
                        ...
                        if (!batchActive)
                        {
                            ...
                            combinedStart = start;
                            combinedCount = count;
                            expectedNextStart = start + count;
                            batchActive = true;
                        }
                        else if (start == expectedNextStart)
                        {
                            // Extend current batch
                            combinedCount += count;
                            expectedNextStart += count;
                        }
                        else
                        {
                            // Non-contiguous: flush and restart
                        ...
                        // If this is a partial buffer, force flush after it
                        if (!isFull)
                        {
                            _sizeAndIndexes2[0] = combinedCount;
                            _sizeAndIndexes2[1] = combinedStart;
                    ...
                    // Final flush if batch is still active
                    if (batchActive && combinedCount > 0)
                    {
                        _sizeAndIndexes2[0] = combinedCount;
                        _sizeAndIndexes2[1] = combinedStart;
                        _CS_CameraVisibility.SetInts(GPUIConstants.PROP_sizeAndIndexes2, _sizeAndIndexes2);
